...
const ThucPham = () => {
  ...
  const fetchDataa = async () => {
    try {
      const response = await axios.get('http://10.0.2.2:3004/api4/getListthucpham');
  ...
  const renderProductRows = () => {
    ...
    for (let i = 0; i < thucpham.length; i += itemsPerRow) {
      const rowItems = thucpham.slice(i, i + itemsPerRow);
      const row = (
  ...
  return (
    <View style={styles.productsContainer}>
      ...
      <Text style={styles.productsTitle}>SẢN PHẨM THỰC PHẨM </Text>
      {isLoading ? (
        <Text>Loading...</Text>
      ) : (
        <ScrollView>
