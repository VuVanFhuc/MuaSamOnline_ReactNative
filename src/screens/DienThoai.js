...
const DienThoai = () => {
  ...
  const fetchDataa = async () => {
    try {
      const response = await axios.get('http://192.168.55.104:3003/api3/getListdienthoai');
  ...
  const renderProductRows = () => {
    ...
    for (let i = 0; i < dienthoai.length; i += itemsPerRow) {
      const rowItems = dienthoai.slice(i, i + itemsPerRow);
      const row = (
  ...
  return (
    <View style={styles.productsContainer}>
      ...
      <Text style={styles.productsTitle}>SẢN PHẨM ĐIỆN THOẠI </Text>
      {isLoading ? (
        <Text>Loading...</Text>
      ) : (
        <ScrollView>
